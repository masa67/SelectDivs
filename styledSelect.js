...
var styledSelect = (function () {
    ...
    function renderHtml(cfg) {
        ...
        html  = '<div class="styled-select-label">' + cfg.label + '</div>' +
                '<div class="styled-select-value-list">' +
        ...
        for (i = 0; i < cfg.options.length; i += 1) {
            el = cfg.options[i];
            html +=     '<div class="styled-select-list-item"' +
                            ' data-value="' + el.value + '"' +
    ...
    function create(cfg) {
        ...
        (function () {
            ...
            valueField.addEventListener('click', function (e) {
                e.stopPropagation(); // prevent 'blur', see body click below
                selectListField = this.parentElement.getElementsByClassName('styled-select-list')[0];
                selectListField.style.display = (selectListField.style.display !== 'block') ? 'block' : 'none';
            }, false);
